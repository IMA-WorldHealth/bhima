...
async function getItemsMetadata(params) {
  ...
  const sql = `
   SELECT BUID(inventory.uuid) as uuid, inventory.code, inventory.text AS label, iu.abbr AS unit,
      ...
      (SELECT GROUP_CONCAT(BUID(t2.uuid), ';', t2.name, ';', t2.color
       ORDER BY t2.name SEPARATOR '|') FROM tags AS t2
       JOIN inventory_tag itag2 ON itag2.tag_uuid = t2.uuid
       WHERE itag2.inventory_uuid = inventory.uuid) AS tag_details,
