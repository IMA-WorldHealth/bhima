...
function getDupes(req, res, next) {
  ...
  if (!('label' in options)) {
    ...
    // PLACEHOLDER: May want to implement the following as a special search
    //   query = `
    //   SELECT
    //     BUID(l.uuid) AS uuid, l.label, l.quantity, l.initial_quantity,
    //     l.unit_cost, l.description, l.entry_date, l.expiration_date,
    ...
    //   FROM lot l
    //   JOIN inventory i ON i.uuid = l.inventory_uuid
    //   GROUP BY label, inventory_uuid, quantity, initial_quantity, unit_cost,
    //        description, expiration_date HAVING num_duplicates > 1
    //   ORDER BY inventory_text, num_duplicates, label
    // `;
  }
