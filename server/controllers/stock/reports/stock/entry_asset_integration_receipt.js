const {
  _, ReportManager, Stock, NotFound, db, barcode, identifiers, STOCK_ENTRY_ASSET_INTEGRATION_TEMPLATE,
 ...
 *
 * GET /receipts/stock/entry_asset_integration/:document_uuid
 */
async function stockEntryAssetIntegrationReceipt(documentUuid, session, options) {
  const data = {};
  const optionReport = _.extend(options, { filename : 'STOCK.RECEIPTS.ENTRY_ASSET_INTEGRATION' });
  const autoStockAccountingEnabled = session.stock_settings.enable_auto_stock_accounting;

  // set up the report with report manager
  const report = new ReportManager(STOCK_ENTRY_ASSET_INTEGRATION_TEMPLATE, session, optionReport);
  ...
  const sql = `
    SELECT i.code, i.text, BUID(m.document_uuid) AS document_uuid,
      ...
      dm.text as document_reference, ast.abt_inventory_no, ast.origin, ast.purchase_order, ast.vendor, ast.condition
    FROM stock_movement m
    JOIN lot l ON l.uuid = m.lot_uuid
    JOIN inventory i ON i.uuid = l.inventory_uuid
    JOIN inventory_group ig ON ig.uuid = i.group_uuid
    JOIN depot d ON d.uuid = m.depot_uuid
    JOIN lot_asset AS ast ON ast.lot_uuid = l.uuid
    JOIN user u ON u.id = m.user_id
    LEFT JOIN document_map dm ON dm.uuid = m.document_uuid
    WHERE m.is_exit = 0 AND m.flux_id = ${Stock.flux.FROM_ASSET_INTEGRATION} AND m.document_uuid = ?
...
module.exports = stockEntryAssetIntegrationReceipt;
