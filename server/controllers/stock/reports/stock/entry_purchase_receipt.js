...
async function stockEntryPurchaseReceipt(documentUuid, session, options) {
  ...
  const poSql = `
     ...
     `;
  const poLots = await db.exec(poSql, [db.bid(poUuid)]);

  // Count the total number of items to see if the order is complete
  const poTotalQuantity = poLots.reduce((accum, lot) => accum + lot.quantity, 0);
  ...
  rows.forEach(row => {
    ...
    const poLot = poLots.find(p => p.inventory_uuid === row.inventory_uuid);
  ...
  data.details.p_total_cost_ent = data.details.p_total_cost / exRate; // Enterprise currency

  data.details.total_cost_deliv_ent = data.details.total_cost_deliv / exRate; // Enterprise currency

  data.details.p_shipping_handling_ent = data.details.p_shipping_handling / exRate; // Enterprise currency
