...
async function getLotsDepot(depotUuid, params, finalClause) {
  ...
  const inventoriesWithManagementData = await getBulkInventoryCMM(
    resultFromProcess,
    params.monthAverageConsumption,
    params.averageConsumptionAlgo,
  );
...
async function getInventoryQuantityAndConsumption(params, monthAverageConsumption, averageConsumptionAlgo) {
  ...
  // add the CMM
  filteredRows = await getBulkInventoryCMM(filteredRows, setting.month_average_consumption, averageConsumptionAlgo);
