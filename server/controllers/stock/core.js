...
async function getAssets(params, finalClause) {
  ...
  if ('scan_start_date' in params) {
    const startDate = moment(params.scan_start_date).format('YYYY-MM-DD HH:MM:SS');
    scanPeriod = `scan.created_at >= DATE('${startDate}')`;
  }
  if ('scan_end_date' in params) {
    if (scanPeriod) {
      scanPeriod += ' AND ';
    }
    const endDate = moment(params.scan_end_date).format('YYYY-MM-DD HH:MM:SS');
    scanPeriod += `scan.created_at <= DATE('${endDate}')`;
  }
  if (scanPeriod) {
