...
async function updateQuantities(req, res, next) {
  ...
  const findQRec = 'SELECT id FROM `cost_center_allocation_basis_value` WHERE `cost_center_id` = ? AND `basis_id` = ?';
  const updateQRec = 'UPDATE `cost_center_allocation_basis_value` SET `quantity` = ? WHERE `id` = ?';
  const insertQRec = 'INSERT INTO `cost_center_allocation_basis_value` (`cost_center_id`, `basis_id`, `quantity`) VALUES (?, ?, ?)';
