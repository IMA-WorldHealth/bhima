...
function find(options) {
  const sql =
    `
    SELECT 
      p.id, p.fiscal_year_id, p.number, p.start_date, p.end_date, CONCAT(p.start_date, '/',  p.end_date) AS label
    FROM period AS p `;
  ...
  filters.custom('number', `p.number NOT IN (${PERIOD_0_NUM}, ${PERIOD_13_NUM})`);
