...
function computeSubTotals(accounts) {
  ...
  accounts.forEach(acct => {
    if (acct.type_id === INCOME) {
      ...
      acct.deviationPct = ((acct.credit || acct.debit) && acct.budget && acct.budget !== 0)
        ? Math.round(100.0 * ((acct.actuals - acct.budget) / acct.budget)) : null;
  ...
  incomeTotal.deviationPct = (budgetTotal !== 0)
    ? Math.round(100.0 * ((actualsTotal - budgetTotal) / budgetTotal)) : null;
  ...
  summaryTotal.deviationPct = (summaryTotal.budget !== 0)
    ? Math.round(100.0 * ((summaryTotal.actuals - summaryTotal.budget) / summaryTotal.budget)) : null;
...
async function computeBudgetTotalsYTD(accounts, fiscalYearId) {
    ...
    if ((acct.type_id === INCOME) || (acct.type_id === EXPENSE)) {
      ...
      acct.period.forEach(p => {
        ...
        acct.deviationYTDPct = (budgetYTD !== 0)
          ? Math.round(100.0 * ((acct.actuals - budgetYTD) / budgetYTD)) : null;
      });
  ...
  if (incomeTotalRow) {
    ...
    incomeTotalRow.deviationYTDPct = incomeTotalYTD !== 0
      ? Math.round(100.0 * (incomeTotalRow.differenceYTD / incomeTotalRow.budgetYTD)) : null;
  }
  ...
  if (expensesTotalRow) {
    ...
    expensesTotalRow.deviationYTDPct = expensesTotalYTD !== 0
      ? Math.round(100.0 * (expensesTotalRow.differenceYTD / expensesTotalRow.budgetYTD)) : null;
  }
  ...
  if (summaryTotalRow) {
    ...
    summaryTotalRow.deviationYTDPct = expensesTotalYTD !== 0
      ? Math.round(100.0 * (summaryTotalRow.differenceYTD / summaryTotalRow.budgetYTD)) : null;
  }
  return accounts;
}
