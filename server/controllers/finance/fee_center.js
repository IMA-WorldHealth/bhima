...
async function getAllCostCenterAccounts() {
  ...
  const sql = `
    SELECT fc.id AS fee_center_id, fc.label AS fee_center_label, fc.is_principal, rfc.is_cost, a.id AS account_id,
      a.label AS accountLabel, a.number As accountNumber, ritem.is_exception, a.type_id
    FROM fee_center AS fc
    JOIN reference_fee_center AS rfc ON rfc.fee_center_id = fc.id
        ...
        accountConfigCostCenter.push([
          {
            ...
            cost_center_id : account_cost_center.fee_center_id,
            principal_center_id : account_cost_center.is_principal ? account_cost_center.fee_center_id : null,
          }
        ]);
      }
    })
