 ...
 * This function first looks for all accounts in an reference account associated with an auxiliary cost center,
...
async function auxilliary(params) {
  ...
  const filters = new FilterParser(params, { tableAlias : 'cost_center' });
  const filters2 = new FilterParser(params, { tableAlias : 'cost_center' });

  const sql1 = `
    SELECT cost_center.id, cost_center.label, cost_center.is_principal, reference_cost_center.account_reference_id,
    reference_cost_center.is_cost, reference_cost_center.is_variable, reference_cost_center.is_turnover,
    account_reference_item.account_id, account.number, account_reference_item.is_exception, account.type_id
    FROM cost_center
    JOIN reference_cost_center ON reference_cost_center.cost_center_id = cost_center.id
    JOIN account_reference_item
      ON account_reference_item.account_reference_id = reference_cost_center.account_reference_id
    JOIN account ON account.id = account_reference_item.account_id
  `;

  filters.equals('is_principal');
  filters.equals('cost_center_id', 'id', 'cost_center');
  filters.equals('typeCostCenter', 'is_cost', 'reference_cost_center');
  ...
  filters2.equals('typeCostCenter', 'is_cost', 'reference_cost_center');
