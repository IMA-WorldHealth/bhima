function configuration(data) {
  ...
  data.costCenter.forEach(center => {
    center.cost_debit = 0;
    ...
    data.references.forEach(reference => {
      if (center.id === reference.cost_center_id) {
  ...
  configured.principalCostCenters = data.costCenter.filter(item => {
  ...
  configured.principalProfitCenters = data.costCenter.filter(item => {
    return item.is_principal && item.hasProfitReference;
  });

  configured.auxiliaryCostCenters = data.costCenter.filter(item => {
    return !item.is_principal && (item.hasProfitReference || item.hasCostReference);
  });
  ...
  configured.auxiliaryProfitCenters = data.costCenter.filter(item => {
    return !item.is_principal && item.hasProfitReference;
  });
