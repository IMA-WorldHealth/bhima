...
function report(req, res, next) {
  ...
  if (breakEvenProject) {
    ...
    if (params.type) {
      getEncounters = `
        SELECT sfc.cost_center_id, f.label AS costCenter, count(pv.uuid) AS numberOfCases, p.id AS project_id,
      ...
      getEncounters = `
        SELECT sfc.cost_center_id, f.label AS costCenter, pr.name AS project_name, pr.id AS project_id,
  ...
  if (breakEvenProject) {
    q.all(dbPromises)
      .spread((costCenter, projects, references, accountReferences, dataDistributions, encounters, allocationKey) => {
        const config = {
          costCenter,
          references,
          accountReferences,
