...
test.describe('Inventory Configuration', () => {
  ...
  const group = {
    ...
    cogs_account  : 'Achat Médicaments en comprimés',
    expires : 1,
    deprecation_rate : 0,
    unique_item : 0,
  };
  ...
  test.describe('Groups', () => {
    ...
    test('creates a new inventory group', async () => {
      ...
      await components.accountSelect.set(group.sales_account, null, TU.locator('[data-sales-account]'));
      await components.accountSelect.set(group.stock_account, null, TU.locator('[data-stock-account]'));
      await components.accountSelect.set(group.cogs_account, null, TU.locator('[data-cogs-account]'));

      await TU.input('$ctrl.session.depreciation_rate', group.deprecation_rate);
    ...
    test('creates an inventory group with only a sales account', async () => {
      ...
      await components.accountSelect.set(groupWithOnlySalesAccount.sales_account, null,
        TU.locator('[data-sales-account]'));

      await TU.input('$ctrl.session.depreciation_rate', groupWithOnlySalesAccount.deprecation_rate);
