...
describe('(/depots) The depots API ', () => {
  ...
  it('GET /depots/:uuid/inventories/:uuid/cmm returns the CMM for a depot', async () => {
    ...
    try {
      ...
      const rows = await db.exec('SELECT BUID(uuid), start_date, end_date, quantity, in_quantity, out_quantity, BUID(inventory_uuid), BUID(depot_uuid)  FROM stock_movement_status;');
