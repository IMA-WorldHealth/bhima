...
describe.skip('Tree Navigation', () => {
  ...
  it('remembers the currently selected node', async () => {
    ...
    expect(await selected.getAttribute('data-unit-key')).to.eventually.equal('TREE.FISCAL_YEAR');
    ...
    expect(await selected.isPresent()).to.eventually.equal(true);
    expect(await selected.getAttribute('data-unit-key')).to.eventually.equal('TREE.FISCAL_YEAR');
  });
  ...
  it('toggles tree nodes open and closed', async () => {
    ...
    expect(await node.$('.fa-folder').isPresent()).to.eventually.equal(true);
    expect(await node.$('fa-folder-open').isPresent()).to.eventually.equal(false);

    // click to open
    await node.click();

    // the open/closed folder icon should be updated
    expect(await node.$('fa-folder-open').isPresent()).to.eventually.equal(true);
    expect(await node.$('fa-folder').isPresent()).to.eventually.equal(false);
