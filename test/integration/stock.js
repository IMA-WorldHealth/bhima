...
describe('(/stock/) The Stock HTTP API', () => {
  ...
  it('GET /stock/lots/movements?is_exit=0&depot_uuid=... returns entries for Depot Principal (10 IN)', () => agent.get(`/stock/lots/movements?is_exit=0&depot_uuid=${depot_principal_uuid}`)
      .then((res) => {
  ...
  let lot_quinine;
  it('GET /stock/lots?lot_uuid=...&depot_uuid=... returns initial quantity of QUININE-A in Depot Principal (100pcs)', () => agent.get(`/stock/lots?lot_uuid=${lot_quinine_uuid}&depot_uuid=${depot_principal_uuid}`)
      .then((res) => {
  ...
  it('GET /stock/lots/movements?is_exit=1&lot_uuid=...&depot_uuid=... returns exit of QUININE-A from Depot Principal (20pcs)', () => agent.get(`/stock/lots/movements?is_exit=1&lot_uuid=${lot_quinine_uuid}&depot_uuid=${depot_principal_uuid}`)
      .then((res) => {
        helpers.api.listed(res, 1);
        let total_exit = 0;
  ...
  it('GET /stock/lots/depots?lot_uuid=...&depot_uuid=... returns remaining quantity of QUININE-A in Depot Principal (80pcs)', () => agent.get(`/stock/lots/depots?lot_uuid=${lot_quinine_uuid}&depot_uuid=${depot_principal_uuid}`)
      .then((res) => {
        helpers.api.listed(res, 1);
