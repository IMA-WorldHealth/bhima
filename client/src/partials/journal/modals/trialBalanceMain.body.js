 ...
 */
function TrialBalanceMainBodyController(Session, journalPostingModalService, Grouping, Columns, Notify, $state, $timeout) {
  var vm = this;
  var columns = [
  ...
  vm.dataByTrans = $state.params.records;
  vm.viewDetail = {
  ...
  function viewSelectedTransaction(accountId) {
    var lines = journalPostingModalService.getRelatedTransaction(accountId, vm.dataByTrans);
    //FIX ME : what is the good way of keeping records? using appcache?
    $state.go('trialBalanceDetail', {lines : lines, feedBack : vm.feedBack, records: records }, {reload : false});
  }
  ...
  function viewErrorReport () {
    ...
    console.log('switching to the trial balance error state');
    $state.go('trialBalanceErrors', {feedBack : vm.feedBack, errors : vm.errors}, {reload : false});
  }
 ...
 window.state = $state;
  ...
  journalPostingModalService.checkTransactions(vm.dataByTrans)
    .then(function(errors) {
      ...
      $state.current.data.checkingData = {errors : errors, feedBack : vm.feedBack, cssClass : vm.cssClass};
