...
function InvoiceItems(InventoryService, Uuid, Store, AppCache) { 
  ...
  function ItemsModel(cacheKey) { 
    var items = this;
    var cacheKey = cacheKey || 'invoice_items_key';
    ...
    function confirmItem(item) { 
      ...
      appcache.put(inventoryItem.uuid, { confirmed : true }); 
    }
    ...
    function removeCache() { 
      ...
      appcache.fetchAll()
        .then(function (items) { 
          items.map(function (item) { appcache.remove(item.key); });
        });
    }
    ...
    function configureBase() { 
      addItem();
      appcache.fetchAll().then(configureCachedItems);
    }
