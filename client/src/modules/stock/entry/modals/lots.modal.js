...
function StockDefineLotsModalController(
  ...
  function onChangeUnitCost() {
    ...
    if ((newUnitCost < medianUnitCost * allowableMinChange)
      || (newUnitCost > medianUnitCost * allowableMaxChange)) {
      const msgParams = { newUnitCost, medianUnitCost, curr : vm.currency.symbol };
      const errMsg1 = newUnitCost > medianUnitCost * allowableMaxChange
        ? $translate.instant('WARNINGS.WARN_UNIT_COST_PROBLEM_TOO_HIGH', msgParams)
        : $translate.instant('WARNINGS.WARN_UNIT_COST_PROBLEM_TOO_LOW', msgParams);
      let errMsg = errMsg1.concat($translate.instant('WARNINGS.WARN_UNIT_COST_REASON'));
      errMsg += $translate.instant('WARNINGS.WARN_UNIT_COST_CONFIRM');
      Modal.confirm(errMsg)
        ...
        .catch(() => {
          ...
          vm.form.setUnitCost(vm.stockLine.unit_cost);
          vm.errors = [$translate.instant(errMsg1)];
          vm.form.$invalid = true;
        });
      return;
    }
