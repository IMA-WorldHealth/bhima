...
function CreateShipmentController(
  ...
  function refreshQuantityAvailable(depot, dateTo = new Date()) {
    return fetchAssignedAssets()
      .then(assets => {
        vm.alreadyAssigned = assets;

        // compute assigned quantites
        const assignmentQuantityMap = vm.alreadyAssigned.reduce((map, asset) => {
        ...
        const usedItems = vm.stockForm._pool.available.data.filter(lot => {
          const assignedQuantity = assignmentQuantityMap[lot.lot_uuid] || 0;

          if (assignedQuantity) {
            lot._quantity_available -= assignedQuantity;
          }

          return lot._quantity_available === 0 && assignedQuantity > 0;
        });
  ...
  function fetchAssignedAssets() {
    const isEdit = !vm.isCreateState && existingShipmentUuid;
    return Shipment.getAssignedAssets({
